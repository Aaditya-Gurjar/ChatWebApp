...
const MessageSend = ({ chatId }) => {
	...
	const handleMediaBox = () => {
		const selectedFile = mediaFile.current?.files[0];
		if (selectedFile) {
			const reader = new FileReader();
			reader.onloadend = () => {
				const base64String = reader.result;
				setBase64Image(base64String); // full string like: data:image/jpeg;base64,...
	...
	return (
		<>
			{mediaBox && (
				<div className="border border-slate-500 rounded-md absolute bottom-[7vh] mb-1 left-2 bg-slate-800 w-60 h-48 z-50">
			...
			>
				<label htmlFor="media" className="cursor-pointer">
					<FaFolderOpen title="Open File" size={22} className="active:scale-75 hover:text-green-400" />
				</label>
				<input
					ref={mediaFile}
					type="file"
				...
				/>
				<span className="flex justify-center items-center">
					{(newMessage?.trim() || base64Image) && !isSendLoading && (
						<button className="outline-none p-2 border-slate-500 border-l" onClick={handleSendMessage}>
							<FaPaperPlane title="Send" size={18} className="active:scale-75 hover:text-green-400" />
						</button>
					)}
					{isSendLoading && (
						<button className="outline-none p-2 border-slate-500 border-l">
							<LuLoader title="loading..." fontSize={18} className="animate-spin" />
						</button>
					)}
				</span>
			</form>
